...
public class PasswordView extends RelativeLayout {
    ...
    private OnForgetPwdClickListener mOnForgetPwdClickListener;
    private OnInputPwdFinishListener mOnInputPwdFinishListener;
    ...
    public PasswordView(Context context, AttributeSet attrs) {
        ...
        tvPwdTitle = (TextView) view.findViewById(R.id.tv_pwd_title);

        tvForgetPwd.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                mOnForgetPwdClickListener.onForgetPwdClick();
            }
        });
        ...
        tvList[5].addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (s.toString().length() == 1) {
                    ...
                    mOnInputPwdFinishListener.onInputPwdFinish(strPassword);    //接口中要实现的方法，完成密码输入完成后的响应逻辑
                }
            }
        });
    }
...
//    }
     ...
     * @param title
     */
    public void setPwdTitle(String title){
        if (!TextUtils.isEmpty(title)) {
            tvPwdTitle.setText(title);
        }
    }

    public void setOnForgetPwdClickListener(OnForgetPwdClickListener listener){
        mOnForgetPwdClickListener = listener;
    }

    public void setOnInputPwdFinishListener(OnInputPwdFinishListener listener){
        mOnInputPwdFinishListener = listener;
    }

    interface OnForgetPwdClickListener{
        void onForgetPwdClick();
    }

    interface OnInputPwdFinishListener{
