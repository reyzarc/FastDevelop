...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private static final int[]         ATTRS                    = new int[] {
            android.R.attr.textColorPrimary, android.R.attr.textSize, android.R.attr.textColor,
    ...
    private boolean                    isIndicatorTextWidth     = false;
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        dividerWidth = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsDividerWidth,
                dividerWidth);
        indicatorHeight = a.getDimensionPixelSize(
                R.styleable.PagerSlidingTabStrip_pstsIndicatorHeight, indicatorHeight);
        isIndicatorTextWidth = a.getBoolean(
                R.styleable.PagerSlidingTabStrip_pstsUserIndicatorTextWidth,
                isIndicatorTextWidth);
        ...
        isPaddingMiddle = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsPaddingMiddle,
                isPaddingMiddle);
        ...
        tabTextSelectedAlpha = a.getFloat(R.styleable.PagerSlidingTabStrip_pstsTextSelectedAlpha,
                OPAQUE);
        a.recycle();
    ...
    private void addTab(final int position, CharSequence title, View tabView) {
        ...
        tabsContainer.addView(linearLayout, position,
    ...
    private Pair<Float, Float> getIndicatorCoordinates() {
        ...
        float nextTabRight;
        View currentTab;
        LinearLayout currentTab1;
        if (!isIndicatorTextWidth) {
            // default: line below current tab
            currentTab = tabsContainer.getChildAt(currentPosition);
            lineLeft = currentTab.getLeft();
            lineRight = currentTab.getRight();
        } else {
            // default: line below current tab
            currentTab1 = (LinearLayout) tabsContainer.getChildAt(currentPosition);
            lineLeft = currentTab1.getChildAt(0).getLeft()
                    +  currentTab1.getLeft();
            lineRight = currentTab1.getChildAt(0).getRight()
                    +  currentTab1.getLeft();
        }
        ...
        if (currentPositionOffset > 0f && currentPosition < tabCount - 1) {
            ...
            if (!isIndicatorTextWidth) {
                currentTab = tabsContainer.getChildAt(currentPosition + 1);
                nextTabLeft = currentTab.getLeft();
                nextTabRight = currentTab.getRight();

            } else {
                currentTab1 = (LinearLayout) tabsContainer.getChildAt(currentPosition + 1);
                nextTabLeft = currentTab1.getChildAt(0).getLeft() + currentTab1.getLeft();
                nextTabRight = currentTab1.getChildAt(0).getRight() + currentTab1.getLeft();
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Pair<Float, Float> lines = getIndicatorCoordinates();
        canvas.drawRect(lines.first + padding, height - indicatorHeight, lines.second + padding,
                height, rectPaint);
        // draw underline
        rectPaint.setColor(underlineColor);
        canvas.drawRect(padding, height - underlineHeight, tabsContainer.getWidth() + padding,
                height, rectPaint);
        // draw divider
        if (dividerWidth != 0) {
            dividerPaint.setStrokeWidth(dividerWidth);
    ...
    private class PageListener implements OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            ...
            int offset = tabCount > 0
                    ? (int) (positionOffset * tabsContainer.getChildAt(position).getWidth()) : 0;
